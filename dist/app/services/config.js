 ...
 * 3. defaultValue
 */
export const getConfig = async (key, defaultValue) => {
    try {
 ...
 */
export const syncConfigToEnv = async () => {
    try {
...
export const setConfig = async (key, value, description) => {
    const config = await AppConfig.findOneAndUpdate({ key }, { value, description, isActive: true }, { upsert: true, new: true });
