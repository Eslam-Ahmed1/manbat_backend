...
const buildPublicPlantQuery = async (query) => {
    const dbQuery = {};
    if (query.search) {
    ...
    if (query.category) {
        ...
        const category = await Category.findOne(isObjectId
            ? { _id: query.category }
...
export const getPlants = async (query) => {
    const dbQuery = await buildPublicPlantQuery(query);
    const result = await paginate(Plant, dbQuery, {
        page: query.page,
        limit: query.limit,
