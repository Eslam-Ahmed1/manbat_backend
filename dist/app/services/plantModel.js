...
export const getPlantModelConfig = async () => {
    ...
    const diseaseConfidenceThreshold = detectionSettings.plantModel.diseaseConfidenceThreshold;
    const timeoutMs = parseInt((await getConfig("PLANT_MODEL_TIMEOUT_MS", "30000")) ?? "30000", 10);
...
export const parseHfDiseaseLabel = (label) => {
 ...
 */
export const resolveDiseaseDbName = async (modelLabel) => {
...
const normalizeHfResponse = async (data) => {
    ...
    if (!dbDiseaseName) {
        console.warn(`   ⚠️ [SCAN] HF label not mapped to DB: "${data.disease}" — Gemini fallback`);
        return {
            plantType: parsed.plantType,
            isSupportedPlant: true,
            isHealthy: false,
 ...
 */
export const predictWithPlantModel = async (imageBuffer, mimeType, config) => {
    ...
    try {
        ...
        if (!response.ok) {
            console.warn(`   ⚠️ [SCAN] Plant model HTTP ${response.status} — fallback to Gemini`);
            return null;
        }
        ...
        if (!prediction)
            return null;
        console.log(`   ✅ [SCAN] HF model (${Date.now() - t0}ms) — label: ${prediction.rawLabel ?? "—"}, db: ${prediction.diseases[0]?.name ?? "healthy"}, confidence: ${prediction.confidence}`);
        return prediction;
    }
    catch (error) {
...
export const isPlantSupported = (prediction, config) => {
    if (prediction.modelMessage)
        return false;
    if (!prediction.isSupportedPlant)
        return false;
    if (!config.supportedPlants.length)
        return true;
