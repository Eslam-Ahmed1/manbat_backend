...
const normalizeTreatmentKey = (name) => name
    .toLowerCase()
    .replace(/\b(application|applications|treatment|treatments|program|spray)\b/g, "")
    .replace(/[^\w\s]/g, "")
    .replace(/\s+/g, " ")
    .trim();
/** أسماء علاجات عامة من AI لا تُعرض إذا وُجد علاج متجر للمرض */
export const isGenericTreatmentName = (name) => {
    const n = name.trim().toLowerCase();
    if (!n || n === "none identified")
        return true;
...
const pickBetterScanEntry = (a, b) => {
    if (a.hasProducts !== b.hasProducts)
        return a.hasProducts ? a : b;
 ...
 */
export const optimizeScanDetectedEntries = (entries) => {
    if (!entries.length)
        return entries;
    ...
    for (const entry of existingEntries) {
        const key = entry.disease._id?.toString() ?? entry.disease.name.toLowerCase();
        if (!byDisease.has(key))
            byDisease.set(key, []);
        byDisease.get(key).push(entry);
    }
    const optimized = [...newEntries];
...
export const findDiseaseByName = async (name) => {
...
export const persistNewDiseaseWithTreatment = async (ai) => {
    ...
    const skipTreatment = !treatmentName || treatmentName.toLowerCase() === "none identified";
    if (skipTreatment) {
    ...
    if (!isGenericTreatmentName(treatmentName)) {
        const treatmentRegex = new RegExp(`^${escapeRegex(treatmentName)}$`, "i");
        let treatment = await Treatment.findOne({
            name: { $regex: treatmentRegex },
        });
        if (treatment) {
            if (!treatment.disease_ids.some((id) => id.toString() === disease._id.toString())) {
...
export const buildNewDiseaseScanEntry = (disease, treatment) => ({
...
export const getTreatmentsWithProductsForDiseaseIds = async (diseaseIds) => {
    ...
    for (const disease of diseases) {
        ...
        const linkedTreatments = treatments.filter((t) => t.disease_ids.some((id) => id.toString() === diseaseId));
...
export const getTreatmentsByDiseaseId = async (diseaseId) => {
    const disease = await Disease.findById(diseaseId);
    if (!disease)
        throw new appError("Disease not found", 404);
    return await Treatment.find({ disease_ids: diseaseId });
};
export const getTreatmentsByDiseaseIds = async (diseaseIds) => {
