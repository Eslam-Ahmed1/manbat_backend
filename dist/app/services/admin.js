...
export const createProduct = async (imageBuffer, productData) => {
...
export const updateProduct = async (id, imageBuffer, productData) => {
    const updatePayload = {};
    if (productData.productName !== undefined)
        updatePayload.name = productData.productName;
    if (productData.description !== undefined)
        updatePayload.description = productData.description;
    if (productData.price !== undefined)
        updatePayload.price = productData.price;
    if (productData.quantity !== undefined)
        updatePayload.quantity = productData.quantity;
    if (productData.discount !== undefined)
        updatePayload.discount = productData.discount;
    if (productData.categoryName) {
        const category = await ProductCategory.findOne({ name: { $regex: productData.categoryName, $options: 'i' } });
        if (!category)
            throw new appError('Category name does not exist', 400);
...
export const updateDisease = async (diseaseId, diseaseData) => {
    ...
    const disease = await Disease.findByIdAndUpdate(diseaseId, diseaseData, { new: true, runValidators: true });
    if (!disease)
...
export const getAllDiseases = async (query = {}) => {
    const result = await paginate(Disease, {}, {
        page: query.page,
        limit: query.limit,
        sort: { name: 1 }
    });
...
export const updateTreatment = async (treatmentId, treatmentData) => {
    ...
    const treatment = await Treatment.findByIdAndUpdate(treatmentId, treatmentData, { new: true, runValidators: true }).populate('disease_ids');
    if (!treatment)
...
export const getAllTreatments = async (query = {}) => {
...
export const createPlant = async (imageBuffer, plantData) => {
    let categoryId = null;
    if (plantData.categoryName) {
        const category = await Category.findOne({ name: plantData.categoryName });
        if (!category)
            throw new appError('Category name does not exist or is incorrect', 400);
...
export const updatePlant = async (plantId, imageBuffer, plantData) => {
    const updatePayload = {};
    if (plantData.name)
        updatePayload.name = plantData.name;
    if (plantData.categoryName) {
        const category = await Category.findOne({ name: plantData.categoryName });
        if (!category)
            throw new appError('Category name does not exist or is incorrect', 400);
...
export const getAllPlants = async (query = {}) => {
    const result = await paginate(Plant, {}, {
        page: query.page,
        limit: query.limit,
        populate: 'category_id',
        sort: { name: 1 }
...
const buildArticlePayload = async (articleData) => {
    const payload = {};
    if (articleData.title)
        payload.title = articleData.title;
    if (articleData.summary !== undefined)
        payload.summary = articleData.summary;
    if (articleData.content)
        payload.content = articleData.content;
    if (articleData.tags)
        payload.tags = articleData.tags;
...
export const createArticle = async (imageBuffer, articleData) => {
...
export const updateArticle = async (articleId, imageBuffer, articleData) => {
...
export const getAllScans = async (query = {}) => {
    const result = await paginate(PlantScan, {}, {
        page: query.page,
        limit: query.limit,
        populate: [
...
export const runDatabaseSeeds = async () => {
    ...
    try {
        ...
        logs.push(stdout);
        if (stderr)
            logs.push(`Warning/Error: ${stderr}`);
        return {
            success: true,
