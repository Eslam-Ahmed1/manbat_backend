...
const runSeeds = async () => {
    ...
    const treatmentOps = allTreatmentsData.map(t => {
        const disease_ids = t.diseaseNames
            .map(name => {
                ...
                if (!id) { console.log(`   ⚠️  Disease not found: "${name}"`); treatmentWarnings++; }
                return id;
            })
            .filter(Boolean);
        return {
            updateOne: {
                filter: { name: t.name },
                update: { $set: { name: t.name, instructions: t.instructions, disease_ids: disease_ids as mongoose.Types.ObjectId[] } },
                upsert: true
            }
        };
    });
    ...
    const productOps = allProductsData.map(p => {
        ...
        if (p.treatmentName && !treatment_id) { console.log(`   ⚠️  Treatment not found: "${p.treatmentName}"`); productWarnings++; }
        if (p.categoryName && !product_category_id) { console.log(`   ⚠️  Category not found: "${p.categoryName}"`); productWarnings++; }
        const doc = {
