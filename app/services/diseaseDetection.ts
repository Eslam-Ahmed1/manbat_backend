...
import {
    ...
    predictWithPlantModel,

    type PlantModelConfig,
...
const dedupeByName = (diseases: DetectedDiseaseInput[]): DetectedDiseaseInput[] => {

    const seen = new Map<string, DetectedDiseaseInput>();
...
const getDefaultTreatmentForDisease = async (diseaseId: string) => {
...
const mapCustomModelToDetectedDiseases = async (
    ...
    const diseases: DetectedDiseaseInput[] = [];
    ...
    for (const item of prediction.diseases) {
        ...
        const { treatment, instructions } = await getDefaultTreatmentForDisease(

            diseaseRecord._id.toString(),
...
const canUseCustomModelOnly = (

    prediction: PlantModelPrediction,

    config: PlantModelConfig,

    mapped: { diseases: DetectedDiseaseInput[]; allKnownInDb: boolean },

): boolean => {

    if (prediction.modelMessage) return false;

    if (!isPlantSupported(prediction, config)) return false;

    if (prediction.confidence < config.confidenceThreshold) return false;



    if (prediction.isHealthy) return true;



    const confidentDiseases = prediction.diseases.filter(

        (d) => d.confidence >= config.diseaseConfidenceThreshold,

    );

    if (!confidentDiseases.length) return false;
...
const buildMetaFromPlantModel = (

    prediction: PlantModelPrediction,

    source: DetectionMeta["source"],
...
const tryPlantModel = async (

    imageBuffer: Buffer,

    mimeType: string,

    plantConfig: PlantModelConfig,
    ...
    if (!plantConfig.enabled || !plantConfig.url) {
        ...
        if (mode === "plant_model_only") {
            ...
            throw new appError(
                ...
                "Plant model is not configured. Set PLANT_MODEL_URL via admin scan-detection settings.",
    ...
    if (!prediction) {
        ...
        if (mode === "plant_model_only") {
            ...
            throw new appError(
                ...
                "Plant disease model is unavailable. Try again later.",
    ...
    if (!prediction.modelMessage) {
        ...
        if (canUseCustomModelOnly(prediction, plantConfig, mapped)) {
            ...
            console.log(
                ...
                `   ✅ [SCAN] HF model (${Date.now() - t0}ms) — mode: ${mode}`,
        ...
        if (mode === "plant_model_only") {
            ...
            throw new appError(
                ...
                `Plant model could not produce a final diagnosis: ${reason}`,
        ...
        console.log(
            ...
            `   ↪️ [SCAN] HF inconclusive (hybrid) — Gemini fallback`,
...
export const detectDiseasesFromImage = async (
    ...
    const withMode = (meta: DetectionMeta): DetectionMeta => ({ ...meta, detectionMode: mode });
    ...
    if (mode === "gemini_only") {
        ...
        const diseases = dedupeByName(

            await detectDiseasesWithGemini(imageBuffer, mimeType),
    ...
    const plantAttempt = await tryPlantModel(
    ...
    const geminiDiseases = dedupeByName(
